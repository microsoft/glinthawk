  ...
  , epoll_fd_num( epoll_fd.fd_num() )
{
  ...
  CHECK_SYSCALL( "epoll_ctl", ::epoll_ctl( epoll_fd_num, EPOLL_CTL_ADD, this->fd.fd_num(), *this ) );
}
...
EventLoop::Result EventLoop::wait_next_event( const int timeout_ms )
{
  ...
  for ( auto it = _fd_rules.begin(); it != _fd_rules.end(); ) {
    ...
    if ( rule.current_in_interested != in_interested or rule.current_out_interested != out_interested ) {
      ...
      CHECK_SYSCALL( "epoll_ctl", epoll_ctl( _epoll_fd.fd_num(), EPOLL_CTL_MOD, rule.fd.fd_num(), rule ) );
    }
  ...
  // (writeable/readable)
  {
    ...
    available_fd_count = CHECK_SYSCALL(
