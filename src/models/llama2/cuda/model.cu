  ...
  const std::vector<std::shared_ptr<ContextType>>& contexts )
{
  ...
  for ( size_t i = 0; i < inference_states.size(); i++ ) {
    ...
    if ( contexts[i]->empty() ) {
      ops::CHECK_CUDA( cudaMemcpy( activations.data() + 2 * Config::dim * sizeof( DType ),
                                   this->state_.k + i * Config::kv_dim,
                                   Config::kv_dim * sizeof( DType ),
                                   cudaMemcpyDeviceToHost ) );

      ops::CHECK_CUDA( cudaMemcpy( activations.data() + ( 2 * Config::dim + Config::kv_dim ) * sizeof( DType ),
  ...
  const std::vector<std::shared_ptr<ContextType>>& contexts )
{
    ...
    switch ( inference_states[i].dtype() ) {
      case DataType::Float16:
        ...
        ops::cvt_and_copy_to_cuda(
          this->state_.q + i * Config::dim, reinterpret_cast<__half*>( activation_data ) + Config::dim, Config::dim );
        ...
        if ( activation_len > 2 * Config::dim * DataTypeSize( inference_states[i].dtype() ) ) {
          ops::cvt_and_copy_to_cuda(
            ...
            reinterpret_cast<__half*>( activation_data ) + 2 * Config::dim,
            Config::kv_dim * 2 );
        }
        break;
  ...
  for ( size_t i = 0; i < inference_states.size(); i++ ) {
    ...
    switch ( inference_states[i].dtype() ) {
      case DataType::Float16:
        ops::cvt_and_copy_from_cuda( reinterpret_cast<__half*>( activations.data() ) + Config::dim,
                                     this->state_.xb + i * Config::dim,
                                     Config::dim );
        break;
      case DataType::Float32:
        ops::cvt_and_copy_from_cuda( reinterpret_cast<float*>( activations.data() ) + Config::dim,
                                     this->state_.xb + i * Config::dim,
                                     Config::dim );
        break;
      default: throw std::runtime_error( "invalid dtype" );
  ...
  std::vector<InferenceState>&& inference_states )
{
  ...
  for ( size_t i = 0; i < inference_states.size(); i++ ) {
    ...
    ops::CHECK_CUDA( cudaMemcpy( this->state_.x + i * Config::dim,
                                 inference_states[i].activations().data(),
                                 Config::dim * sizeof( DType ),
                                 cudaMemcpyHostToDevice ) );
