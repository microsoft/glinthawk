...
int main( int argc, char* argv[] )
{
  ...
  try {
    ...
    for ( size_t i = 0; i < num_tries; i++ ) {
      ...
      ops::CHECK_CUDA( cudaMemcpy( cuda_pointer + i * dim, cpu_pointer + i * dim, dim * sizeof( uint8_t ), cudaMemcpyHostToDevice ) );
    }
    ...
    for ( size_t i = 0; i < num_tries; i++ ) {
      ...
      ops::CHECK_CUDA( cudaMemcpy( cpu_pointer + i * dim, cuda_pointer + i * dim, dim * sizeof( uint8_t ), cudaMemcpyDeviceToHost ) );
    }
