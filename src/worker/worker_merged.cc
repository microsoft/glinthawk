...
void WorkerMerged<Model_GPU, Model_CPU>::setup_stats_handler()
{
  ...
  event_loop_.add_rule(
    ...
    Direction::In,
    stats_timer_,
    bind( &WorkerMerged<Model_GPU, Model_CPU>::handle_stats, this ),
    [] { return true; },
    [] { LOG( ERROR ) << "Stats timer stopped."; } );
}

template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::setup_peer( std::map<net::Address, Peer>::iterator peer_it )
{
  peer_it->second.message_handler.install_rules(
    this->event_loop_,
    this->rule_categories_,
    bind( &WorkerMerged<Model_GPU, Model_CPU>::handle_peer_message, this, placeholders::_1 ),
  ...
  event_loop_.add_rule(
    ...
    [this, peer_it] {
      for ( auto& state : peer_it->second.outgoing_states ) {
        ...
        LOG(INFO) << "[EVENT]," << std::chrono::steady_clock::now().time_since_epoch().count() << "," << state.to_log() << ",serialized_begin";
        ...
        LOG(INFO) << "[EVENT]," << std::chrono::steady_clock::now().time_since_epoch().count() << "," << state.to_log() << ",serialized_end";
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::setup_blobstore( const string& blobstore_uri )
{
                                                               ...
                                                               const bool randomize_cpu )
{
  ...
  compute_kernel_ = make_unique<compute::ComputeKernelMerged<Model_GPU, Model_CPU>>(
    make_unique<Model_GPU>(
      model_root, start_layer_gpu, end_layer_gpu, max_concurrency_size_gpu, max_context_count_gpu, randomize_gpu ),
    concurrency_size_pre_attention_gpu,
    concurrency_size_attention_gpu,
    concurrency_size_post_attention_gpu,
    concurrency_size_classification_gpu,
    start_layer_gpu,
  ...
  , model_root_( model_root )
{
  ...
  coordinator_.message_handler.install_rules(
    this->event_loop_,
    this->rule_categories_,
    bind( &WorkerMerged<Model_GPU, Model_CPU>::handle_coordinator_message, this, placeholders::_1 ),
  ...
  event_loop_.add_rule(
    ...
    Direction::In,
    listen_socket_,
    bind( &WorkerMerged<Model_GPU, Model_CPU>::listen_callback, this ),
    [] { return true; },
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::listen_callback()
{
...
template<typename Model_GPU, typename Model_CPU>
bool WorkerMerged<Model_GPU, Model_CPU>::handle_coordinator_message( core::Message&& msg )
{
  ...
  switch ( msg.opcode() ) {
    case Message::OpCode::InitializeWorker: {
      ...
      setup_compute_kernel( model_root_,
                            ...
                            proto.randomize(),
                            proto.concurrency_att_cpu_size(),
                            proto.max_context_count_cpu(),
                            proto.randomize_cpu() );
    ...
    case Message::OpCode::PushDummyPrompts: {
      ...
      for ( size_t i = 0; i < prompt_count; i++ ) {
        ...
        InferenceState state { get_datatype<typename Model_GPU::ModelDataType>() };
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::handle_compute_kernel_event()
{
  ...
  while ( this->compute_kernel_->pop( state ) ) {
    ...
    LOG(INFO) << "[EVENT]," << std::chrono::steady_clock::now().time_since_epoch().count() << "," << state.to_log() << ",kernel_peer_it";
...
template<typename Model_GPU, typename Model_CPU>
bool WorkerMerged<Model_GPU, Model_CPU>::handle_peer_message( core::Message&& msg )
{
  ...
  switch ( msg.opcode() ) {
    case Message::OpCode::InferenceState: {
      ...
      LOG(INFO) << "[EVENT]," << std::chrono::steady_clock::now().time_since_epoch().count() << "," << state.to_log() << ",peer_push";
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::handle_stats()
{
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::run()
{
  while ( event_loop_.wait_next_event( -1 ) != EventLoop::Result::Exit ) {}
}

template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::prompt_preparation_thread_func()
{
  while ( running_ ) {
    vector<InferenceState> states {};
    PromptID prompt_id;
...
template<typename Model_GPU, typename Model_CPU>
void WorkerMerged<Model_GPU, Model_CPU>::completion_commit_thread_func()
{
  while ( running_ ) {
...
template<typename Model_GPU, typename Model_CPU>
WorkerMerged<Model_GPU, Model_CPU>::~WorkerMerged()
{
...
// template class WorkerMerged<models_gpu::Llama2_7B_Chat<glinthawk::float16_t>,
//                             models_cpu::Llama2_7B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Llama2_13B_Chat<glinthawk::float16_t>,
//                             models_cpu::Llama2_13B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Llama2_70B_Chat<glinthawk::float16_t>,
//                             models_cpu::Llama2_70B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Stories_110M<glinthawk::float16_t>,
//                             models_cpu::Stories_110M<glinthawk::float16_t>>;

// template class WorkerMerged<models_gpu::Llama2_7B_Chat<glinthawk::float32_t>,
//                             models_cpu::Llama2_7B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Llama2_13B_Chat<glinthawk::float32_t>,
//                             models_cpu::Llama2_13B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Llama2_70B_Chat<glinthawk::float32_t>,
//                             models_cpu::Llama2_70B_Chat<glinthawk::float16_t>>;
// template class WorkerMerged<models_gpu::Stories_110M<glinthawk::float32_t>,
//                             models_cpu::Stories_110M<glinthawk::float16_t>>;

template class WorkerMerged<models_gpu::Llama2_7B_Chat<glinthawk::float16_t>,
                            models_cpu::Llama2_7B_Chat<glinthawk::float32_t>>;
template class WorkerMerged<models_gpu::Llama2_13B_Chat<glinthawk::float16_t>,
                            models_cpu::Llama2_13B_Chat<glinthawk::float32_t>>;
template class WorkerMerged<models_gpu::Llama2_70B_Chat<glinthawk::float16_t>,
                            models_cpu::Llama2_70B_Chat<glinthawk::float32_t>>;
template class WorkerMerged<models_gpu::Stories_110M<glinthawk::float16_t>,
                            models_cpu::Stories_110M<glinthawk::float32_t>>;
