...
template<typename Model>
class VirtualPreallocatingContextManager
{
public:
  using StateType = glinthawk::models::BatchedInferenceState<typename Model::ConfigType>;

  VirtualPreallocatingContextManager( const size_t slots );

  bool get_context( const PromptID& prompt_id );
  bool get_contexts( const StateType& state );

  bool release_context( const PromptID& prompt_id );

  size_t free() const { return free_contexts_; }
  size_t allocated() const { return allocated_contexts_.size(); }
  size_t total() const { return free() + allocated(); }
...
private:
  ...
  // TODO(pouya): not sure if vector is the best data structure for an element presence test
  std::vector<PromptID> prompts_with_allocated_contexts_ {};
};

template<typename Model>
VirtualPreallocatingContextManager<Model>::VirtualPreallocatingContextManager( const size_t slots )
  : free_contexts_( slots )
{
  prompts_with_allocated_contexts_.reserve( slots );
}

template<typename Model>
bool VirtualPreallocatingContextManager<Model>::get_context( const PromptID& prompt_id )
{
  std::lock_guard lock { mutex_ };

  auto it = std::find(prompts_with_allocated_contexts_.begin(), prompts_with_allocated_contexts_.end(), prompt_id );
  if ( it != prompts_with_allocated_contexts_.end() ) {
    return true;
  }
  ...
  prompts_with_allocated_contexts_.push_back( prompt_id );
  free_contexts_--;
  return true;
}

template<typename Model>
bool VirtualPreallocatingContextManager<Model>::get_contexts( const StateType& state )
{
  size_t no_context_count = 0;
  std::vector<bool> allocated;
  allocated.resize( state.batch_size(), false );
  ...
  // (1) let's make sure we can assign contexts to all the prompts first
  for ( size_t i = 0; i < state.batch_size(); i++ ) {
    const auto it = std::find(prompts_with_allocated_contexts_.begin(), prompts_with_allocated_contexts_.end(), state.prompt_id( i ) );
    if ( it == prompts_with_allocated_contexts_.end() ) {
      no_context_count++;
    } else {
      allocated[i] = true;
  ...
  for ( size_t i = 0; i < state.batch_size(); i++ ) {
    if ( not allocated[i] ) {
      prompts_with_allocated_contexts_.push_back( state.prompt_id( i ) );
      free_contexts_--;
    }
  }
...
template<typename Model>
bool VirtualPreallocatingContextManager<Model>::release_context( const PromptID& prompt_id )
{
  std::lock_guard lock { mutex_ };
  ...
  free_contexts_++;
  prompts_with_allocated_contexts_.erase( it );
  return true;
}
