...
class Coordinator:
    ...
    def create_routing_message(self):
        ...
        # assuming layers per worker is 1
        ...
        for layer in range(self.model.n_layers):
            ...
            sub_msg.ip = socket.inet_ntoa(self.layer_workers[layer+1][0].ip)
            sub_msg.port = self.layer_workers[layer][0].port
            message.layer_to_address.append(sub_msg)
        # Classification
        sub_msg = glinthawk_pb.SetRoute.LayerToAddress()
        ...
        sub_msg.ip = socket.inet_ntoa(self.layer_workers[0][0].ip)
        sub_msg.port = self.layer_workers[0][0].port
        message.layer_to_address.append(sub_msg)
    ...
    async def message_processor(self):
        while True:
            ...
            if message.opcode in [Message.OpCode.HeyCPU, Message.OpCode.HeyGPU]:
                ...
                worker.role_idx = self.ip_port_to_index[worker.ip]
                ...
                if message.opcode == Message.OpCode.HeyCPU:
                    if worker.role_idx < self.model.n_layers / self.model.layers_per_worker:
                        worker.start_layer = worker.role_idx * self.model.layers_per_worker
                        worker.end_layer = (worker.role_idx + 1) * self.model.layers_per_worker - 1
                    ...
                    if worker.role_idx == 0:
                        ...
                        worker.start_layer = (worker.role_idx - 1) * self.model.layers_per_worker
                        worker.end_layer = worker.role_idx * self.model.layers_per_worker
                        worker.max_concurrency_size_pre = self.concurrency_size_pre
                        worker.max_concurrency_size_att = 0
                ...
                if message.opcode == Message.OpCode.HeyCPU:
                    ...
                    if worker.role_idx not in self.layer_workers:
                        self.layer_workers[worker.role_idx] = [None, None]
                    self.layer_workers[worker.role_idx][0] = worker
...
def main(listen_address, listen_port, **kwargs):
    assert kwargs['layers-per-worker'] == 1
